...
// Inititalise params.
__global__
void d_init(int *d_nVars, int *d_lasso, int *d_done,
	int *d_step, int l)
{
...
__global__
void d_check(int *d_ctrl, int *d_step, int *d_nVars, int M,
	int Z, int l, int *d_done, int *d_towrite)
{
	int ind = threadIdx.x + blockIdx.x * blockDim.x;
	if (ind < l && !d_done[ind]) {
		int	n = min(M - 1, Z - 1);
